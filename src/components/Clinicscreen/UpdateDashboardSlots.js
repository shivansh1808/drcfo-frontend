...
const UpdateDashboardSlots = () => {
  ...
  let { slotid, clinicid } = useParams();
  // console.log(slotid, clinicid);
  const navigate = useNavigate();
  useEffect(() => {
    // fetch(
    //   `https://drco-all-backend-617u.onrender.com/get/slot?clinicId=${clinicid}&slotId=${slotid}`
    // )
    //   .then((res) => res.json())
    //   .then((data) => {
    //     console.log(data);
    ...
    //   });
      axios.get(`/get/slot?clinicId=${clinicid}&slotId=${slotid}`).then((res)=>{
  ...
  const handleSubmit = async (e) => {
    ...
    const slots = { slotId: slotid, clinicId: clinicid, slot: appointment };
    console.log(slots);
...
export default UpdateDashboardSlots;

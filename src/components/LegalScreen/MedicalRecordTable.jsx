import moment from "moment";
import React, { useEffect, useState } from "react";
import { getAppointments } from "../../api/doctor";
import { calculateAge, makeAddressString } from "../../util";
import { appointmentFilter } from "../../util/constants";
import useAppContext from "../context/AppContext";
import MedicalRecordTableSkeleton from "./MedicalRecordTableSkeleton";

export default function MedicalRecordTable() {
	const { openSnackbar } = useAppContext();
	const [loading, setLoading] = useState();
	const [medicalRecords, setMedicalRecords] = useState([]);
	const [responseData, setResponseData] = useState(null);

	function onClickExportPdf() {}

	function onClickExportExcel() {}

	function onClickFilter() {}

	function onClickExpandButton() {}

	async function onComponentLoad() {
		setLoading(true);
		const response = await getAppointments(appointmentFilter.completed);
		if (response?.data?.length) {
			setResponseData(response.data);
		} else {
			openSnackbar({
				severity: "error",
				message: response?.message || "Could not load appoinments",
			});
		}
		setLoading(false);
	}

	function handleResponseData() {
		if (Array.isArray(responseData)) {
			setMedicalRecords(
				responseData.map((appointment) => {
					const followUpDate =
						appointment?.patient?.child?.date ||
						appointment?.unregisteredPatient?.child?.date;
					return {
						date: moment(appointment?.date).format("DD/MM/YYYY") || "null",
						name:
							appointment?.patient?.name ||
							appointment?.unregisteredPatient?.name ||
							"null",
						age:
							calculateAge(
								appointment?.patient?.dateOfBirth ||
									appointment?.unregisteredPatient?.dateOfBirth
							) || "null",
						gender:
							appointment?.patient?.gender ||
							appointment?.unregisteredPatient?.gender ||
							"null",
						address:
							makeAddressString(
								appointment?.patient?.address ||
									appointment?.unregisteredPatient?.address
							) || "null",
						occupation:
							appointment?.patient?.occupation ||
							appointment?.unregisteredPatient?.occupation ||
							"null",
						followUp: followUpDate
							? moment(followUpDate).format("DD/MM/YYYY")
							: "No Follow-Up",
						medicalHistory:
							appointment?.patient?.medicalHistory ||
							appointment?.unregisteredPatient?.medicalHistory ||
							"null",
					};
				})
			);
		}
	}

	useEffect(() => {
		onComponentLoad();
	}, []);

	useEffect(() => {
		handleResponseData();
	}, [responseData]);

	return (
		<div class="legal_container2">
			<div className="d-flex justify-content-between align-items-center legal_topBar">
				<h2>Medical Record</h2>
				<div className="d-flex align-items-center">
					<div className="export_dropdown">
						<p
							className="btn btn-primary dropdown-toggle export_btn "
							role="button"
							id="dropdownMenuLink"
							data-bs-toggle="dropdown"
							aria-expanded="false"
						>
							Export
						</p>
						<ul className="dropdown-menu" aria-labelledby="dropdownMenuLink">
							<li onClick={onClickExportPdf} className="dropdown-item">
								Export as PDF
							</li>
							<li onClick={onClickExportExcel}>Export as XLS</li>
						</ul>
					</div>

					<div>
						<button className="date_filter_btn" onClick={onClickFilter}>
							<svg
								width="20"
								height="20"
								viewBox="0 0 28 28"
								fill="none"
								xmlnsXlink="http://www.w3.org/1999/xlink"
							>
								<rect width="28" height="28" fill="url(#pattern0)" />
								<defs>
									<pattern
										id="pattern0"
										patternContentUnits="objectBoundingBox"
										width="1"
										height="1"
									>
										<use
											xlinkHref="#image0_4862_2818"
											transform="scale(0.0111111)"
										/>
									</pattern>
									<image
										id="image0_4862_2818"
										width="90"
										height="90"
										xlinkHref="data:image/png;base64,"
									/>
								</defs>
							</svg>
							{/* {recordfilterOpen && (
                <ClickOutHandler onClickOut={recordclickOut}>
                  <DatePicker
                    selected={startDate}
                    onChange={onChange}
                    startDate={startDate}
                    endDate={endDate}
                    selectsRange
                    inline
                    calendarClassName="legal_calender_range"
                  />
                </ClickOutHandler>
              )} */}
						</button>
					</div>
					<div className="expand_btn" onClick={onClickExpandButton}>
						<svg
							width="30"
							height="30"
							viewBox="0 0 29 30"
							fill="none"
							xmlns="http://www.w3.org/2000/svg"
						>
							<path
								d="M20.5858 0H8.4287C3.14807 0 0 3.14807 0 8.4287V20.5713C0 25.8664 3.14807 29.0145 8.4287 29.0145H20.5713C25.8519 29.0145 28.9999 25.8664 28.9999 20.5858V8.4287C29.0145 3.14807 25.8664 0 20.5858 0ZM24.2996 11.6058C24.2996 12.2006 23.8064 12.6938 23.2116 12.6938C22.6168 12.6938 22.1235 12.2006 22.1235 11.6058V8.4287L8.4287 22.1235H11.6058C12.2006 22.1235 12.6938 22.6168 12.6938 23.2116C12.6938 23.8064 12.2006 24.2996 11.6058 24.2996H5.80289C5.65782 24.2996 5.51275 24.2706 5.38218 24.2126C5.11502 24.0978 4.90211 23.8849 4.78739 23.6178C4.74104 23.4873 4.71653 23.35 4.71485 23.2116V17.4087C4.71485 16.8139 5.20809 16.3206 5.80289 16.3206C6.39769 16.3206 6.89093 16.8139 6.89093 17.4087V20.5858L20.5858 6.89093H17.4087C16.8139 6.89093 16.3206 6.39769 16.3206 5.80289C16.3206 5.20809 16.8139 4.71485 17.4087 4.71485H23.2116C23.3566 4.71485 23.4872 4.74386 23.6323 4.80189C23.8934 4.91795 24.111 5.12105 24.2271 5.39669C24.2706 5.52725 24.2996 5.65782 24.2996 5.80289V11.6058Z"
								fill="#91A7FF"
							/>
						</svg>
					</div>
					{/* <MedicalRecordExpanded
            recordIsOpen={recordIsOpen}
            setRecordIsOpen={setRecordIsOpen}
            startDate={startDate}
            endDate={endDate}
            exportPDF={exportPDF}
            setrecordfilterOpen={setrecordfilterOpen}
            recordfilterOpen={recordfilterOpen}
            datefrom={datefrom}
            dateto={dateto}
            onChange={onChange}
            Appendix3filterData={Appendix3filterData}
          /> */}
				</div>
				{/* {startDate && (
          <p className="legal_reset_btn" onClick={reset}>
            Reset
          </p>
        )} */}
			</div>
			<ul class="legal_responsive_table">
				<li class="table-header">
					<div class="col_1">S. No.</div>
					<div class="col_2">Date</div>
					<div class="col_3">Name</div>
					<div class="col_4">Age</div>
					<div class="col_5">Gender</div>
					<div class="col_6">Address</div>
					<div class="col_7">Occupation</div>
					<div class="col_8">Follow-Up</div>
					<div class="col_9">Medical History</div>
				</li>
				{loading ? (
					<MedicalRecordTableSkeleton />
				) : (
					medicalRecords.map((item, index) => {
						const followDateString = item?.followUpdate;
						const followUpdate = new Date(followDateString?.slice(0, -1));
						const followDateDay = followUpdate.getDate();
						const followDateYear = followUpdate.getFullYear();
						const followMonth = moment(followUpdate).format("MMM");
						const displayFollowUpdate =
							followMonth?.substring(0, 3) +
							" " +
							followDateDay +
							", " +
							followDateYear;
						return (
							<li class="table-row" key={item._id}>
								<div class="col_1">{index + 1}</div>
								<div class="col_2">{item?.date}</div>
								<div class="col_3">{item?.name}</div>
								<div class="col_4">{item?.age}</div>
								<div class="col_5">{item?.gender}</div>
								<div class="col_6">{item?.address}</div>
								<div class="col_7">{item?.occupation}</div>
								<div class="col_7">{item?.followUp}</div>
								<div class="col_9 history_link">Read here</div>
							</li>
						);
					})
				)}
			</ul>
		</div>
	);
}
